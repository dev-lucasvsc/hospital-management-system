...
export default function App() {
  ...
  const handleLogout = () => {
    // Remove o token JWT do localStorage ao sair
  ...
  const telaAnterior = cargo === 'MEDICO' ? 'medico' : cargo === 'ADMIN' ? 'admin' : 'recepcao';
  if (tela === 'monitor') return <MonitorTV aoVoltar={() => setTela(telaAnterior)} />;
    ...
    <div style={{ display: 'flex', height: '100vh', background: 'var(--bg-base)', overflow: 'hidden' }}>
      ...
      <aside style={{
        ...
        <div style={{ padding: '18px 16px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: 9, background: 'linear-gradient(135deg, #4f46e5, #7c3aed)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 15, flexShrink: 0, boxShadow: '0 0 16px rgba(79,70,229,0.3)' }}>🏥</div>
          <div>
        ...
        <nav style={{ flex: 1, padding: '10px 8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {itensVisiveis.map(item => (
            ...
            onMouseLeave={e => { if (tela !== item.id) (e.currentTarget.style.background = 'transparent'); }}>
              ...
              {item.label}
              {tela === item.id && (
                <span style={{ marginLeft: 'auto', width: 6, height: 6, borderRadius: '50%', background: 'var(--accent)', animation: 'pulse-dot 2s ease-in-out infinite' }}/>
              )}
            </button>
          ))}
        </nav>
        ...
        <div style={{ padding: '12px 16px', borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--bg-hover)', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, flexShrink: 0 }}>
          ...
          <button onClick={handleLogout} title="Sair" style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 15, padding: 4, borderRadius: 6, transition: 'color var(--transition)' }}
      ...
      {/* Conteúdo */}
      <main style={{ flex: 1, overflow: 'auto', padding: '28px 32px', background: 'var(--bg-base)', display: 'flex', justifyContent: 'center' }}>
