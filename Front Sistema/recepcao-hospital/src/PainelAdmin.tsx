...
export function PainelAdmin() {
  ...
  const carregar = async () => { const res = await api.get('/funcionarios'); setFuncionarios(res.data); };
  useEffect(() => { carregar(); }, []);
  ...
  const cadastrar = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/funcionarios/cadastrar', { ...novo, cpf: novo.cpf.replace(/\D/g,'') });
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      <div className="card" style={{ padding: '20px', marginBottom: 20 }}>
        <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 16 }}>Novo acesso</div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '12px 14px' }}>
          ...
          <div><label className="label">CPF</label><input className="input" name="cpf" value={novo.cpf} onChange={handleChange} placeholder="000.000.000-00" /></div>
          <div><label className="label">Nascimento</label><input className="input" name="dataNascimento" value={novo.dataNascimento} onChange={handleChange} placeholder="DD/MM/AAAA" /></div>
          <div><label className="label">Senha</label><input className="input" type="password" name="senha" value={novo.senha} onChange={handleChange} /></div>
          {novo.cargo==='MEDICO' && (
            <div><label className="label">CRM</label><input className="input" name="registroProfissional" value={novo.registroProfissional} onChange={handleChange} /></div>
          )}
        </div>
        <div style={{ marginTop: 16, display:'flex', justifyContent:'flex-end' }}>
          <button className="btn btn-primary" onClick={cadastrar} disabled={loading}>{loading ? 'Criando...' : '+ Criar acesso'}</button>
        </div>
      </div>

      <div className="card" style={{ padding: 0, overflow:'hidden' }}>
        <table className="table">
          <thead><tr><th>Matrícula</th><th>Nome</th><th>CPF</th><th>Cargo</th></tr></thead>
          <tbody>
            {funcionarios.map(f => {
              ...
              return (
                <tr key={f.id}>
                  ...
                  <td><span style={{ display:'inline-flex', alignItems:'center', padding:'3px 10px', borderRadius:99, fontSize:11, fontWeight:600, background: style.bg, color: style.color }}>{f.cargo}</span></td>
                </tr>
              );
            })}
          </tbody>
