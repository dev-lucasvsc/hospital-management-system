...
export function Login({ aoLogar }: { aoLogar: (cargo: string, nome: string) => void }) {
  ...
  return (
    <div style={{
      ...
      backgroundImage: 'radial-gradient(ellipse 60% 50% at 50% -10%, rgba(79,70,229,0.12) 0%, transparent 70%)',
    }}>
      <div className="fade-up" style={{ width: '100%', maxWidth: '360px', padding: '0 16px' }}>
          ...
          <form onSubmit={entrar} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div>
              ...
              <input className="input" type="number" value={userId}
                onChange={e => setUserId(e.target.value)} placeholder="Ex: 1" required autoFocus />
            </div>
            <div>
        ...
        <p style={{ textAlign: 'center', marginTop: 20, fontSize: 12, color: 'var(--text-muted)' }}>
          UNIEURO · Projeto Integrador ADS
        </p>
      </div>
    </div>
  );
